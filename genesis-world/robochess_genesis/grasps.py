...
``assets/chess/generated/<scale>/grasps/chess_grasps.json`` were generated for the Franka
panda hand.  That is not the limitation it looks like: a chess grasp is a top-down pinch of
a shaft, which is a property of the *piece* and of parallel-jaw geometry, not of a
particular hand.  So the franka_panda grasps are reused for every arm and simply
re-expressed in that arm's end-effector frame -- which is exactly what
:meth:`GenesisRobotSpec.graspgen_to_ee` encodes.

Two things happen here, both ported from ``lab/scripts/generate_chess_pick_demos.py``
through ``newton/robochess_newton/grasps.py``:

* :class:`GraspLibrary` loads the JSON, retargets every candidate into the arm's
  ``ee_link`` frame and spins the yaw-free pieces (everything but the knight is a solid of
  revolution, so any rotation about the piece axis is an equally valid grasp and multiplies
  12 candidates into 192);
* :class:`GraspPlanner` re-scores those candidates against the board as it actually stands
  at the start of the episode.  GraspGen scored each piece in isolation, and the best grasp
  of a lone bishop is frequently the one that rakes the open fingers through the queen next
  to it.  The neighbours are modelled as upright cylinders and the 24-point probe cloud of
  the open gripper is tested against them, at the grasp pose *and* at the pre-grasp pose
  90 mm back along the approach, because the fingers have to get there too.

This module is **engine-free**: plain numpy on the host, no Genesis import, so it is a
line-for-line translation of the Newton port rather than a reinterpretation of it.  It runs
once per episode per world (about 300 us for 192 candidates against 31 neighbours), not per
tick.  Quaternions are ``(x, y, z, w)`` and poses are 4x4 row-major homogeneous matrices,
matching the JSON; see :mod:`robochess_genesis.gsmath` for where that meets Genesis' wxyz.
"""
...
# board_layout first: it installs PXR_WORK_THREAD_LIMIT, and it is the module that owns the
# asset paths this one reads.
...
if TYPE_CHECKING:  # pragma: no cover - typing only
    from .robots import GenesisRobotSpec
...
A grasp that merely grazes a neighbour still knocks it over once the fingers close, so the
penalty has to start before contact does."""

COLLISION_WEIGHT = 40.0
"""Score charged per metre of worst-case penetration [1/m].

1 mm of overlap costs 0.04 of rank score and 25 mm costs 1.0, which is more than the entire
spread of rank scores -- i.e. a deep collision always loses to a merely mediocre grasp, and
a sub-millimetre graze breaks ties."""
...
A carry sized for a Panda puts a shorter arm's wrist outside its workspace, where the IK
gives up and the piece is dropped mid-transfer."""

PLACE_APPROACH_HEIGHT = 0.12
"""Fallback height above the destination at which the carry ends [m].

The Isaac Lab script hard-codes this, and it is a trap on the wider boards: the destination
itself is always empty, but the *path* to it is not.  The carry is sized by
:func:`carry_height` to clear the tallest piece standing (191 mm with a king on the board),
while ``transfer`` interpolates straight from the lift to the pre-place -- so a fixed 120 mm
turns the carry into a descending sweep that ends with the carried piece's base 124 mm up,
16 mm *below* the top of every king it still has to cross.

:func:`place_goals` therefore takes the carry height as its ``approach_height`` and this
constant is only the floor that :func:`carry_height` already enforces."""
...
def load_piece_geometry(scale_tag: str = bl.PIECE_SCALE_TAG) -> dict[str, dict[str, float]]:
    ...
    Read straight from the JSON rather than through :class:`assets.PieceAssets`, which would
    transcode the collision meshes for numbers that are two floats per kind.
    """
...
def gripper_probe_points(spec: GenesisRobotSpec) -> np.ndarray:
    """Sample points on the two open fingers, in the ``ee_link`` frame. ``(24, 3)``.

    A slab per finger, running back from the TCP along the approach axis and offset to
    either side along the closing axis.  Crude, but it is only used to ask "would these
    fingers dip into a neighbouring piece", and it needs no per-arm mesh -- which matters
    because two of the four arms are URDF imports with no usable finger geometry to sample.
    """
...
class GraspLibrary:
    ...
    def __init__(self, grasp_file: str | Path, spec: GenesisRobotSpec, max_candidates: int = 12) -> None:
        grasp_file = Path(grasp_file)
        if not grasp_file.exists():
            raise FileNotFoundError(
                f"Missing GraspGen grasps at {grasp_file}. They are committed through git-lfs "
                f"(git lfs pull); regenerating them needs GraspGen's own interpreter (see "
                f"lab/Readme.md):\n"
        ...
        # The one place the arm enters: GraspGen's (approach +Z, closing +X, origin at the
        # gripper base) frame carried onto this arm's ee_link frame.
    ...
    def candidates(self, kind: str, num_yaws: int) -> tuple[np.ndarray, np.ndarray]:
        ...
        The yaw rotation is applied on the **left**: the matrices live in the piece frame
        and every piece except the knight is a solid of revolution about its own ``+Z``, so
        spinning the piece frame is what generates equivalent grasps.  Score index ordering
        is ``yaw * C + candidate``, matching the tiled scores.
        """
...
def carry_height(max_piece_height: float, reach: float) -> float:
    ...
    The carried piece hangs below the hand with its base at the lift height, so the carry
    has to clear the tallest piece still standing -- a fixed lift leaves a pawn's base level
    with the top of the king and drags it off the board.
    """
...
class GraspPlan:
    ...
    """4x4 pose of ``ee_link`` at the grasp."""
...
class GraspPlanner:
    ...
    def __init__(
        ...
        # Cache: the candidate set depends only on (kind, num_yaws), and planning happens
        # once per world per episode, so this saves a 192x4x4 matmul chain every reset.
    ...
    def approach_directions(self, hand_quat: np.ndarray) -> np.ndarray:
        ...
        The Isaac Lab implementation uses the ``ee`` frame's local **+Z** here, which is the
        approach axis for three of the four arms but the *lateral* axis for the reBot (whose
        ``gripper_end`` reaches along its own +X).  On the reBot that backs the pre-grasp off
        sideways instead of along the approach, which is a documented latent bug there;
        rotating ``spec.approach_axis`` is the generalisation, and is bit-identical for
        franka, piper and yam.
        """
    ...
    def plan(
        ...
        *piece_pose_w* is the commanded piece's 4x4 world pose, *other_centers* the
        ``(O, 3)`` world base-plane positions of every other piece and *other_kinds* their
        kinds.  Nothing else on the scene is checked -- not the board, not the table, not the
        arm's own links -- exactly as in the Isaac Lab version.
        """
        ...
        # The fingers have to reach the grasp as well as hold it, so the pre-grasp cloud is
        # scored too.
        ...
        if len(other_centers):
            ...
            # Each neighbour is an upright column of infinite depth: a probe is inside it
            # when it is both within the (inflated) radius and below the (inflated) top, and
            # the penalty is the smaller of the two margins.
...
def place_goals(
    ...
    Returns ``(hand_in_piece, {"place": ..., "pre_place": ...})``.  The piece is put down
    upright, keeping the yaw it had when it was picked up, which is why the place pose
    cannot be derived from the grasp plan: how the piece actually ended up in the hand is
    only known once it is held.

    *approach_height* is where ``transfer`` ends and ``place`` begins; pass the episode's
    carry height so the carry stays level (see :data:`PLACE_APPROACH_HEIGHT`).
    """
