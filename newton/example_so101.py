...
class Example:
    def __init__(self, viewer, args):
        ...
        so101.default_joint_cfg = newton.ModelBuilder.JointDofConfig(limit_ke=1.0e3, limit_kd=1.0e1, friction=1e-5)
        so101.default_shape_cfg.ke = 1.0e3
        so101.default_shape_cfg.kd = 2.0e2
        so101.default_shape_cfg.kf = 1.0e3
        so101.default_shape_cfg.mu = 0.75

        asset_path = pathlib.Path(__file__).parent.parent

        # so101.add_usd(
        #     str(asset_path / "assets" / "so101" / "TheRobotStudio" / "so101_new_calib" / "so101_new_calib_physx.usd"),
        ...
        so101.add_urdf(
            str(asset_path / "assets" / "so101" / "TheRobotStudio" / "so101_new_calib.urdf"),
        #     xform=wp.transform(wp.vec3(0, 0, 0.2)),
        ...
        for i in range(6, so101.joint_dof_count):
            so101.joint_target_ke[i] = 500.0
            so101.joint_target_kd[i] = 10.0
            so101.joint_target_mode[i] = int(JointTargetMode.POSITION)

        # approximate meshes for faster collision detection
        so101.approximate_meshes("bounding_box")

        builder = newton.ModelBuilder()
        builder.replicate(so101, self.world_count)
